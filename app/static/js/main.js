...
function getActionHtml(ip, port) {
    return `
        <div class="action-div action-panel">
            <div class="flag-box">
                <span class="info-label" style="display:inline; margin:0;">ACCESS_FLAG:</span>
...
function attackGetFlag(ip, port, btn) {
    ...
    const outputSpan = parent.querySelector('.flag-output');
    outputSpan.innerText = 'EXTRACTING...';
    btn.disabled = true;
    ...
    fetch('/api/attack/get_flag', {
        ...
        .then(data => {
            btn.disabled = false;
            if (data.success) {
                outputSpan.innerText = data.output;
            } else {
                outputSpan.innerText = 'ERR: ' + data.output;
            }
        });
}
...
function openXshell(ip, port, user, password) {
    ...
    if (isLocal) {
        ...
        // Client-side: Copy SSH Command
        const sshCmd = `ssh ${user}@${ip} -p ${port}`;
        navigator.clipboard.writeText(sshCmd).then(() => {
            showToast('连接命令已复制: ' + sshCmd);
        }).catch(err => {
            showToast('复制失败: ' + sshCmd);
            prompt('请手动复制连接命令:', sshCmd);
        });
    }
}
